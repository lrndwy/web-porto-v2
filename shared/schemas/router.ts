...
/**
 * The Anthropic Messages shape: a top-level `system` and only user/assistant
 * turns. `.loose()` so every other field (temperature, top_k, metadata, …)
...
/** The wire format a provider speaks; the gateway translates around it. */
export const providerFormatSchema = z.enum(['openai', 'anthropic'])
...
export const providerCreateSchema = z.object({
 ...
 format: providerFormatSchema.default('openai'),
