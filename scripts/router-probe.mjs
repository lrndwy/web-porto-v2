...
async function post(path, { key = KEY, body = {}, raw = false, headers = {} } = {}) {
...
const callRouter = (options = {}) =>
 post('/v1/chat/completions', {
  body: { model: MODEL, messages: [{ role: 'user', content: 'Hello' }], ...(options.body ?? {}) },
  ...options,
 })

const callMessages = (options = {}) =>
 post('/v1/messages', {
  body: { model: MODEL, max_tokens: 64, messages: [{ role: 'user', content: 'Hello' }], ...(options.body ?? {}) },
  ...options,
 })
...
console.log(`Probing ${BASE}/v1 as ${KEY.slice(0, 16)}…\n`)
...
// 1b. The Anthropic-compatible face, translated to the same provider.
{
 const result = await callMessages()
 check('anthropic messages returns 200', result.status === 200, `got ${result.status}`)
 check(
  'body carries content blocks',
  Array.isArray(result.json?.content),
