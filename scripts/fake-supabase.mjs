...
const db = {
 ...
 ai_providers: [
  { id: 'bbbbbbbb-0000-4000-8000-000000000001', name: 'OpenAI', base_url: 'https://api.openai.com/v1', secret_api_key: 'sk-not-a-real-key', format: 'openai', is_active: true, created_at: now(), updated_at: now() },
 ],
 ai_models: [
 ...
 ai_api_keys: [
  // key_hash is the sha256 of key_prefix, so the gateway accepts the seeded key.
  { id: uuid(), key_prefix: 'pk_uhuy_7f3a9cK2mQ8vRtYb4nH6jLpWd5xZc1E', key_hash: 'a050ffe3a1cf0afbdb3354c7c41822794ca8ec600061bf45b9fe51ddafffb650', label: 'Default', is_active: true, last_used_at: '2026-09-17T20:00:00Z', created_at: now(), revoked_at: null },
 ],
 ai_usage_logs: [
...
Bun.serve({
 ...
 async fetch(request) {
  ...
  // The atomic fixed-window counter the router calls. Mirrors the SQL
  // function's contract: `{ allowed, current_count, reset_at }`.
  if (url.pathname === '/rest/v1/rpc/ai_rate_limit_hit') {
   const { p_identifier, p_window_type, p_window_seconds, p_limit } = await request.json()
   const at = Date.now()
   let row = db.ai_rate_limits.find(
    (candidate) => candidate.identifier === p_identifier && candidate.window_type === p_window_type,
   )
   if (!row || Date.parse(row.expires_at) <= at) {
    row = {
     id: uuid(),
     ...
     expires_at: new Date(at + p_window_seconds * 1000).toISOString(),
    }
    db.ai_rate_limits.push(row)
   }
   const allowed = !(p_limit > 0 && row.request_count >= p_limit)
   if (allowed) row.request_count += 1
   return json([{ allowed, current_count: row.request_count, reset_at: row.expires_at }])
  }
